        ...
        for(int j = fh->N_shift[1]; j < (fh->N_md[1] + fh->N_shift[1]); j++)
        {
            for(int i = fh->N_shift[0]; i < (fh->N_md[0] + fh->N_shift[0]); i++)
            {
                ...
                if(fh->scheme == One_Way)
                {
                    ...
                    for(int d = 0; d < DIM; d++)
                    {
                    	arr[C].beta_term[d] = fh->beta*((arr[C].u_fh[d] + arr[C].u_fh_flow[d])*arr[C].ro_fh - arr[C].uro_md[d]);
                    }
                }
                ...
                for(int d = 0; d < DIM; d++)
                {
                    ...
                    for(int du = 0; du < DIM; du++)
                    {
                    	if (fh->S_function == AFM)
                    		arr[Cm].alpha_u_grad[du][d] = arr[Cm].grad_ro[d]*arr[Cm].S*pow((1 - arr[Cm].S),fh->gamma)*arr[Cm].u_md[du];
                    	else
